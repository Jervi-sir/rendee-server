...
1. Write the port 80 portion of the configuration to Nginx:
   ```bash
   sudo nano /etc/nginx/sites-available/rendee.jervi.dev
   ```
...
3. Enable the site and restart Nginx:
   ```bash
   sudo ln -sf /etc/nginx/sites-available/rendee.jervi.dev /etc/nginx/sites-enabled/
   sudo nginx -t
...
```bash
sudo certbot --nginx -d rendee.jervi.dev
```
